const AI2 = {
  async gerarFolha(materia, tema, nivel, topicos) {
    ...
    if (!res.ok)
      throw new Error(data.detail || `Erro ${res.status} no servidor.`);
    return data;
  },
  ...
  _renderVisual(visual) {
    ...
    try {
      ...
      switch (visual.tipo) {
        case "grafico_funcao":
          el = AI2._chartFuncao(visual.dados);
          break;
        case "grafico_barras":
          el = AI2._chartBarras(visual.dados);
          break;
        case "grafico_pizza":
          el = AI2._chartPizza(visual.dados);
          break;
        case "svg":
          el = AI2._renderSVG(visual.codigo);
          break;
        case "imagem_wiki":
          el = AI2._renderWiki(visual.busca);
          break;
        default:
          return null;
      }
  ...
  _renderSVG(codigo) {
    if (!codigo?.trim().toLowerCase().startsWith("<svg"))
    ...
    if (svgParsed) {
      ...
      const geoCount = geoTags.reduce(
        (n, tag) => n + svgParsed.querySelectorAll(tag).length,
        0,
      );
      const rectCount = svgParsed.querySelectorAll("rect").length;
      const textEls = svgParsed.querySelectorAll("text");
      const textLen = Array.from(textEls)
        .map((t) => t.textContent.trim())
        .join("").length;
      if (geoCount === 0 && rectCount <= 2 && textLen > 6) {
        throw new Error(
          "SVG descartado: apenas caixa com texto, sem geometria real.",
        );
      }
    }
    ...
    if (svg) {
      ...
      svg.style.cssText =
        "width:100%;height:auto;display:block;max-height:260px";
    }
    return wrap;
  },
    ...
    for (const lang of ["pt", "en"]) {
      try {
        ...
        const sorted = pages
          ...
          .sort(
            (a, b) => (b.thumbnail?.width ?? 0) - (a.thumbnail?.width ?? 0),
          );
